...
Plugin URI: https://metrotechs.io/plugins/metrotechs-e2w-sync/
...
add_action('plugins_loaded', 'metrotechs_e2w_check_woocommerce_dependency');

// Add activation hook to check for WooCommerce
register_activation_hook(__FILE__, 'metrotechs_e2w_activation_check');

function metrotechs_e2w_activation_check() {
...
function metrotechs_e2w_check_woocommerce_dependency() {
    if (!class_exists('WooCommerce')) {
        add_action('admin_notices', 'metrotechs_e2w_woocommerce_missing_notice');
        return;
    }
    
    // WooCommerce is available, initialize the plugin
    new Metrotechs_E2W_Sync();
}

function metrotechs_e2w_woocommerce_missing_notice() {
...
if (!defined('METROTECHS_E2W_VARIATION_BATCH_SIZE')) {
    define('METROTECHS_E2W_VARIATION_BATCH_SIZE', 10); // Reduced from 50 to 10 for better stability
}

if (!defined('METROTECHS_E2W_CATEGORY_BATCH_SIZE')) {
    define('METROTECHS_E2W_CATEGORY_BATCH_SIZE', 10); // Conservative batch size to prevent critical errors
}

if (!defined('METROTECHS_E2W_PRODUCT_BATCH_SIZE')) {
    define('METROTECHS_E2W_PRODUCT_BATCH_SIZE', 5); // Much smaller batch size to prevent server overload
}

define('METROTECHS_E2W_VERSION', '1.3.5'); // Define version constant

class Metrotechs_E2W_Sync {
    private $options;
    ...
    public function add_admin_menu() {
        ...
        add_menu_page(
            ...
            __('Ecwid2Woo Sync', 'metrotechs-e2w-sync'), // Shorter menu title
            'manage_options',
            $this->settings_slug,
    ...
    public function options_page_router() {
        ...
        wp_enqueue_style('ecwid-wc-sync-admin-css', plugin_dir_url(__FILE__) . 'assets/css/admin-styles.css', [], METROTECHS_E2W_VERSION);
        wp_enqueue_script('ecwid-wc-sync-admin', plugin_dir_url(__FILE__) . 'assets/js/admin-sync.js', ['jquery', 'wp-i18n'], METROTECHS_E2W_VERSION, true);
        ...
        wp_localize_script('ecwid-wc-sync-admin', 'ecwid_sync_params', [
            ...
            'variation_batch_size' => defined('METROTECHS_E2W_VARIATION_BATCH_SIZE') ? METROTECHS_E2W_VARIATION_BATCH_SIZE : 50,
            'i18n' => [
    ...
    public function ajax_batch_sync_DISABLED() {
        ...
        if ($sync_type === 'categories') {
            $default_batch_size = METROTECHS_E2W_CATEGORY_BATCH_SIZE;
        } else {
            $default_batch_size = METROTECHS_E2W_PRODUCT_BATCH_SIZE;
        }
    ...
    private function get_ecwid_store_currency() {
        ...
        $response = wp_remote_get($profile_url, [
            ...
            'headers' => [
                'Accept' => 'application/json',
                'User-Agent' => 'ecwid2woo/' . METROTECHS_E2W_VERSION
            ]
        ]);
...
// Remove direct instantiation - now handled by METROTECHS_E2W_check_woocommerce_dependency()
?>
